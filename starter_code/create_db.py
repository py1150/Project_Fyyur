...
#import sqlalchemy_utils as sqlut
...
def create_db(db_url):
    ...
    if not database_exists(engine.url):
        ...
        print(f'Database {engine.db_url} created')
    else:
        print(f'Database {engine.db_url} exists')
    """    
    #else:
    #    engine.connect()
    #print(f'Database {engine.db_url} created')
...
if __name__=='__main__':
    ...
    # get db_url
    db_url = f'postgresql://{db_user}:{db_pw}@localhost:5432/{dbname}'
    print(f'db_url: {db_url}')

    # create database if it does not exist
    #engine = create_db(db_url=db_url)
    # create engine
    engine = create_engine(db_url, echo=True)

    # create venue table in database
    venue = create_table_venue(engine=engine)
    ...
    with engine.connect() as conn:
        ins = venue.insert().values(venue_data)
...
"""
insert = Venue.insert().values(venue_data)
